...
- `models/schemas.py` — single source of truth for Pydantic models. `Project` carries `format`, `aspect`, `source_filename`, plus `scene_ids` and `quote_ids`. `Scene` and `FormInput` are unchanged from storyboard_maker (Image Posts reuse them verbatim). `QuotePost` parallels `Scene` with `term`, `raw_body`, `summary`, `background_mode`, `background_desc`, optional prompt fields, template selections, and `pending_action: Optional[Literal["summarize", "background"]]` which records the per-card action choice from `SummaryReview` so it survives navigation.
...
- `routers/images.py` — image-post pipeline. Added `POST /api/projects/{id}/scenes/parse` that creates draft Scenes with `form_input.subject` pre-filled from the `ImagePostParser`. `POST /api/projects/{id}/images` passes `project.aspect` to `generate_images` so 16:9 requests aren't silently defaulted to 9:16. `PATCH /api/projects/{id}/scenes/{scene_id}` (`UpdateSceneRequest`) updates a scene's `subject` field on disk; used by the inline entry editor.
- `routers/quotes.py` — quote-post pipeline: `POST .../quotes/parse`, `POST .../quotes/{qid}/summarize`, `POST .../quotes/{qid}/prompt` (Complex only), `POST .../quotes/{qid}/candidates`, `GET .../quotes/{qid}/candidates/{n}`, `POST .../quotes/{qid}/accept`, `GET .../quotes/{qid}/image`, `GET /api/projects/{id}/export` (zip). `accept_quote` only overwrites `template_id`, `color_scheme_id`, and `font_pairing_id` when the request sends non-empty values, preserving previously saved layout selections on re-accepts. `PATCH /api/projects/{id}/quotes/{quote_id}` (`UpdateQuoteRequest`) updates `term`, `raw_body`, and optionally `pending_action` on disk; used by the inline entry editor and by `SummaryReview` to persist per-card action choices.

### Frontend (`frontend/src/`)

- `context/WorkflowContext.tsx` — central state machine, persisted to `localStorage` under key **`shortsmaker_workflow`** (do not reuse the old `storyboard_workflow` key). State carries `format`, `aspect`, `projectName` (defaults to `"Untitled Project"`, auto-set to the uploaded filename), plus quote-specific fields (`quoteId`, `summary`, `backgroundMode`, `backgroundDesc`, `batchMode`) alongside the existing image-post fields. `step` drives which component renders. `SET_PROJECT_NAME` action updates the name and triggers a `PATCH` rename if a project is already saved. `SET_BATCH_MODE` toggles `batchMode` — set to `true` by `SummaryReview`'s "Continue All" path, consumed by `BackgroundChooser`, reset to `false` on completion.
- `api/client.ts` + `api/types.ts` — typed wrappers over `fetch`; all backend calls go through `api.*` functions. Includes `projects.rename(id, name)` and `projects.duplicate(id)` for library management; `images.updateScene(projectId, sceneId, data)` and `quotes.updateQuote(projectId, quoteId, data)` for inline entry editing. `quotes.updateQuote` also accepts `pending_action`. `config.workflow()` fetches `{require_all_marked}` from `/api/config/workflow`.
- `components/EditableProjectName.tsx` — controlled inline text input for the project name. Handles Enter (saves), Escape (reverts), and blur (saves). Used in the top header during creation and in the sidebar during an active workflow. Calls `projects.rename` when a project ID is available.
- Components map 1:1 to workflow steps:
  - `SourcePicker` (SOURCE) — responsive two-column layout on desktop (controls left, file drop right); updates `projectName` from the uploaded filename; includes visual CSS aspect-ratio boxes and platform subtitles.
  - `FormatChooser` (FORMAT) — side-by-side format panels; reads `projectName` from context on project creation.
  - `EntryList` (ENTRY_REVIEW) — interactive inline editor: 2-column card grid, `text-lg` titles and `text-base` body, auto-expanding textareas (no internal scrollbars), auto-saves to backend on blur with a green `✓ Saved` badge via `images.updateScene` / `quotes.updateQuote`.
  ...
  - `SummaryReview` (SUMMARY_REVIEW), `BackgroundChooser` (BACKGROUND_CHOICE), `CardGallery` (CARD_REVIEW), `CollectionView` (COLLECTION) — quote branch. `SummaryReview` shows two action buttons per pending card: **Summarize** and **Choose Background**. Clicking either persists `pending_action` to the backend (fire-and-forget PATCH) and optimistically updates `quotePosts` in state, so the choice survives navigation back from the library. The header offers **Next Card →** (processes one quote through BackgroundChooser → CardGallery) and **Continue All →** (sets `batchMode: true`, takes the user to BackgroundChooser once where they pick settings that are applied to every pending card in sequence — summarizing each as needed, generating candidates, and auto-accepting the first). Both buttons are gated by `require_all_marked` (from `/api/config/workflow`); when `true`, every pending card must have a `pending_action` or an existing `summary` before either button enables. `BackgroundChooser` is dual-column on desktop (background/prompt/model left; typography/color/font right). In batch mode it shows an indigo banner with the pending count and its generate button reads "Apply to All N Cards →"; progress ("Processing card X of N…") is shown inline. Prefills template/scheme/font/mode from the active quote's saved state when resuming a single card. `CardGallery` is click-to-select: clicking a candidate image highlights it with a purple ring; the "Add to Collection ✓" button in the header commits the selection. `CollectionView` re-fetches quotes on mount via `api.quotes.list` so accepted counts and thumbnails stay current.
- `HomePage.tsx` — minimalist landing: "New Project" card + "My Library" card + up to 3 recent projects for quick resume.
- `LibraryPage.tsx` — full project library with tile/list view toggle, inline rename, duplicate, and delete-with-name-confirmation (Radix Dialog). Thumbnails served from the first accepted scene/quote image. Both tile and list views are fully clickable cards (hover shows a `➔` overlay); tile cards use `size='lg'` thumbnails. The ⋮ action menu uses `stopPropagation` so sub-actions don't trigger card open.
- `App.tsx` — top-level page router (`appPage: 'home' | 'library' | 'content'`), not persisted (refresh always returns to home). Syncs app state bidirectionally with URL hash (`#/home`, `#/library`, `#/new-project/{STEP}`, `#/project/{id}/{STEP}`) so browser back/forward and deep links work. Renders `<EditableProjectName />` in the top header. Pre-project and ENTRY_REVIEW/BACKGROUND_CHOICE screens expand to `max-w-5xl`. Sidebar collapses into a sliding overlay drawer on mobile (toggled by a header button); quote card clicks restore the active quote's step (skips to BACKGROUND_CHOICE if already summarized).
...
### Workflow behaviour (`config.yaml`)

```yaml
workflow:
  require_all_marked: true   # when true, SummaryReview's Next Card / Continue All
                             # buttons are disabled until every pending quote has a
                             # pending_action set (or already has a summary).
                             # Set false to restore the old "show Continue immediately" behaviour.
```

`REQUIRE_ALL_MARKED` is loaded at startup by `backend/config.py` and served via `GET /api/config/workflow`.
