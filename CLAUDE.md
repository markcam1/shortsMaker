...
- `routers/quotes.py` — quote-post pipeline: `POST .../quotes/parse`, `POST .../quotes/{qid}/summarize`, `POST .../quotes/{qid}/prompt` (Complex only), `POST .../quotes/{qid}/candidates`, `GET .../quotes/{qid}/candidates/{n}`, `POST .../quotes/{qid}/accept`, `GET .../quotes/{qid}/image`, `GET /api/projects/{id}/export` (zip). `accept_quote` only overwrites `template_id`, `color_scheme_id`, and `font_pairing_id` when the request sends non-empty values, preserving previously saved layout selections on re-accepts. `PATCH /api/projects/{id}/quotes/{quote_id}` (`UpdateQuoteRequest`) updates `term` and `raw_body` on disk; used by the inline entry editor.

### Frontend (`frontend/src/`)

- `context/WorkflowContext.tsx` — central state machine, persisted to `localStorage` under key **`shortsmaker_workflow`** (do not reuse the old `storyboard_workflow` key). State carries `format`, `aspect`, `projectName` (defaults to `"Untitled Project"`, auto-set to the uploaded filename), plus quote-specific fields (`quoteId`, `summary`, `backgroundMode`, `backgroundDesc`) alongside the existing image-post fields. `step` drives which component renders. `SET_PROJECT_NAME` action updates the name and triggers a `PATCH` rename if a project is already saved.
- `api/client.ts` + `api/types.ts` — typed wrappers over `fetch`; all backend calls go through `api.*` functions. Includes `projects.rename(id, name)` and `projects.duplicate(id)` for library management; `images.updateScene(projectId, sceneId, data)` and `quotes.updateQuote(projectId, quoteId, data)` for inline entry editing.
...
- Components map 1:1 to workflow steps:
  ...
  - `SummaryReview` (SUMMARY_REVIEW), `BackgroundChooser` (BACKGROUND_CHOICE), `CardGallery` (CARD_REVIEW), `CollectionView` (COLLECTION) — quote branch. `BackgroundChooser` is dual-column on desktop (background/prompt/model left; typography/color/font right). Prefills template/scheme/font/mode from the active quote's saved state when resuming an existing card. `SummaryReview` shows the Continue button whenever `pendingQuotes.length > 0`; dispatches `SET_QUOTE_POSTS` after summarizing so re-visits reflect the updated summary. `CollectionView` re-fetches quotes on mount via `api.quotes.list` so accepted counts and thumbnails stay current.
